...
import { AUTHOR, company } from './setting';
...
export const getColumn = config => {
  return R.clone(config.header).map((header, key) => {
    let rows = getUniqByIdx({ key, data: config.body });
...
let getRowStartAndEnd = (column, { min, max }) => {
  ...
  if (column.children) {
    ...
    if (first.children) {
      ...
      min = Math.min(first.idx, min);
      max = Math.max(last.idx, max);
    }
  }
  return {
    min,
...
let handleSrcColumn = (src, totalLevel, parentRow = 1) =>
  src.map(item => {
    ...
    let rowInfo = R.pick(['rowId', 'rowEndId', 'title', 'children', 'idx'])(item);
...
const mergeRowInfo = (worksheet, config) => {
  ...
  // 处理前两行表头
  if (config.extra && config.extra.rows) {
...
const createWorkBook = (config: Config) => {
  ...
  if (params.mergev.length) {
    ...
    mergev.forEach(key => {
      // 获取数据项
      let rows = getDataByIdx({ key, data: config.body });
      let start = 0;
